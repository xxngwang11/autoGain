...
private let log = Logger(subsystem: "com.demo.AutoGainExtension", category: "AudioUnitViewController")

@MainActor
public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
    var audioUnit: AUAudioUnit?
    
    var hostingController: HostingController<AutoGainExtensionMainView>?
	...
	*/
	...
	nonisolated public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
		return try DispatchQueue.main.sync {
			...
			audioUnit = try AutoGainExtensionAudioUnit(componentDescription: componentDescription, options: [])
			
			guard let audioUnit = self.audioUnit as? AutoGainExtensionAudioUnit else {
				log.error("Unable to create AutoGainExtensionAudioUnit")
				return audioUnit!
			}
			...
			audioUnit.setupParameterTree(AutoGainExtensionParameterSpecs.createAUParameterTree())
    ...
    private func configureSwiftUIView(audioUnit: AUAudioUnit) {
        ...
        let content = AutoGainExtensionMainView(parameterTree: observableParameterTree)
