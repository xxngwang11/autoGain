...
* /Common - Contains common code split by functionality which should rarely need to be modified. 
	* `Audio Unit/AutoGainExtensionAudioUnit.swift` - A subclass of AUAudioUnit, this is the actual Audio Unit implementation. You may in advanced cases need to change this file to add additional functionality from AUAudioUnit.  
* /Parameters
	* `AutoGainExtensionParameterAddresses.h` - A pure `C` enum containing parameter addresses used by Swift and C++ to reference parameters.
...
* /DSP
	* `AutoGainExtensionDSPKernel.hpp` - A pure C++ class to handle the real-time aspects of the Audio Unit Extension. DSP and processing should be done here. Note: Be aware of the constraints of real-time audio processing. 
* /UI
	* `AutoGainExtensionMainView.swift` - SwiftUI based main view, add your SwiftUI views and controls here.

## Adding a parameter
1. Add a new parameter address to the `AutoGainExtensionParameterAddress` enum in `AutoGainExtensionParameterAddresses.h` 


Example:

```c
typedef NS_ENUM(AUParameterAddress, AutoGainExtensionParameterAddress) {
	sendNote = 0,
	....
	attack
```
...
3. In order to manipulate the DSP side of the Audio Unit we must handle changes to our new parameter in `AutoGainExtensionDSPKernel.hpp`. In the `setParameter` and `getParameter` methods add a case for the new parameter address.
...
```cpp
	void setParameter(AUParameterAddress address, AUValue value) {
		switch (address) {
			....
			case AutoGainExtensionExtensionParameterAddress:: attack:
				mAttack = value;
				break;			
			...
	}
